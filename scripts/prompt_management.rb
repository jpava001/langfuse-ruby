...
#
# IMPORTANT: To link prompts to traces in LangFuse:
# 1. Fetch the prompt object using Langfuse.get_prompt()
# 2. Pass the prompt object to bedrock.invoke() via langfuse_prompt parameter
# 3. This creates the proper link in LangFuse UI for metrics and tracking
#
...
begin
  ...
  Langfuse.propagate_attributes(user_id: Config::Langfuse::USER_ID) do
    ...
    Langfuse.observe("simple-prompt-example", input: { prompt_name: "greeting-assistant" }) do |obs|
      ...
      obs.update(output: { response: result.text })
...
begin
  ...
  Langfuse.propagate_attributes(
    ...
    # Wrap in an observation
    Langfuse.observe("personalized-greeting", input: { prompt_name: "personalized-greeting" }) do |obs|
      # Use with LLM (pass prompt object for linking)
      result = bedrock.invoke(
        model_id: Config::Bedrock::MODEL_ID,
